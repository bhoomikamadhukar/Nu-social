const Users = require('../Login/user');
...
const authCtrl = {
    register: async(req,res)=> {
        try {
            ...
            const user_name = await Users.findOne({usernmae: new UserName})
            if(user_name) return res.status(400).json({msg: "This username already exists"})

            const user_email = await Users.findOne({email})
            if(user_email) return res.status(400).json({msg: "This mail already exists"})

            if(password.length < 6)
            return res.status(400).json({msg: "Password must atleast 6 characters"})
    ...
    login: async(req,res)=> {
        try {
            const { emapil, password } = req.body;
